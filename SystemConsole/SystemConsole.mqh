...
void SystemConsole::ExecuteCommand(string command)
{
	...
	PrintIfTrue(false, __FUNCTION__ + " command: \"" + command + "\" newCommand: \"" + newCommand + "\" context: \"" + context + "\"");
	...
	else if ((command == "[x]exit/[q]quit") || (command == "[x]exit") || (command == "[q]quit") || (command == "x") || (command == "q") || (command == "exit") || (command == "quit"))
	{
		...
		PrintIfTrue(false, __FUNCTION__ + " - Tried to exit");
	}
	...
	else if (!StringIsNullOrEmpty(context))
	{
		if (!StringIsNullOrEmpty(newCommand))
		{
			...
			PrintIfTrue(true, __FUNCTION__ + " Sending to SystemProcessor the command: \"" + newCommand + "\"; context: \"" + context + "\" command: \"" + command + "\"");
		}
	}
}