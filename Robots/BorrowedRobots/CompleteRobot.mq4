...
{              CS="Waiting for next tick .";     // To display comments while testing, simply use CS = .... and
                ...
                hMagic=JenkinsHash(IntegerToString(Magic));
                FileName="B3_"+IntegerToString(Magic)+".dat";
                if(Debug)
                ...
                {              TbF=FileReadInteger(FileHandle,LONG_VALUE);
                                ...
                                {              OTbF=OrderOpenTime();
                                                ...
                                                PhC=FindClosedPL(H);
                                                TradesOpen=true;
                                                if(Debug)Print(FileName+" File Read: "+IntegerToString(TbF)+" Lots: "+DTS(LbF,LotDecimal));
                                }
                                else
                                {              FileDelete(FileName);
                                                TbF=0;
                                                ...
                                                else Print("Error deleting file: "+FileName+" "+IntegerToString(Error)+" "+ErrorDescription(Error));
                                }
                }
                ...
                {              if(y>0)Digit[y,0]=MathPow(10,y);
                                Digit[y,1]=y;
                                if(Debug)Print("Digit: "+IntegerToString(y)+" ["+IntegerToString(Digit[y,0])+","+IntegerToString(Digit[y,1])+"]");
                }
                LabelCreate();
                dLabels=false;
                ...
                //+-----------------------------------------------------------------+
                int GridSet = 0,GridTemp = 0,GridTP = 0,GridIndex = 0,GridLevel = 0,GridError = 0;
                ...
                {               ArrayResize(GridArray,MaxTrades);
                                ...
                                {              if(StringFind(SetCountArray,",")==-1&&GridIndex==0)
                                                ...
                                                {              GridArray[GridLevel,0]=GridTemp;
                                                                ...
                                                                if(Debug)Print("GridArray "+IntegerToString(GridLevel+1)+"  : ["+IntegerToString(GridArray[GridLevel,0])+","+IntegerToString(GridArray[GridLevel,1])+"]");
                                                }
                                                GridIndex=GridLevel;
                                }
                ...
                {              int HolTemp = 0,NumHols = 0,NumBS = 0,HolCounter = 0;
                                string HolTempStr;
                                ...
                                if(NumBS!=NumHols*2)
                                {              Print("Holidays Error, number of back-slashes ("+IntegerToString(NumBS)+") should be equal to 2* number of Holidays ("+IntegerToString(NumHols)+
                ...
                {              if(Period()==43200)sTF="MN1";
                                ...
                                {              for(y=0;y<=2;y++)
                                                ...
                                                if(Debug)Print("Email ["+IntegerToString(z+1)+"] : "+IntegerToString(Email[z]));
                                }
                }
                ...
                //+---------------------------------------------------------------+
                if(SaveStats)
                {              StatFile="B3"+Symbol()+"_"+IntegerToString(Period())+"_"+IntegerToString(EANumber)+".csv";
...
{              int     CbB          =0;     // Count buy
                ...
                double  LbB         = 0.0;     // Count buy lots
                double  LbS         = 0.0;     // Count sell lots
                double  LbT         = 0.0;     // total lots out
                double  OPpBL       = 0.0;     // Buy limit open price
                double  OPpSL       = 0.0;     // Sell limit open price
                double  SLbB        = 0.0;     // stop losses are set to zero if POSL off
                double  SLbS        = 0.0;     // stop losses are set to zero if POSL off
                double  BCb = 0.0,BCh = 0.0,BCa = 0.0;         // Broker costs (swap + commission)
                double  ProfitPot    = 0.0;     // The Potential Profit of a basket of Trades
                double  PipValue = 0.0,PipVal2 = 0.0,ASK,BID = 0.0;
                double  OrderLot = 0.0;
                double  OPbL = 0.0,OPhO = 0.0;           // last open price
                int     OTbL = 0;                // last open time
                double  g2 = 0.0,tp2 = 0.0,Entry = 0.0,RSI_MA = 0.0,LhB = 0.0,LhS = 0.0,LhT = 0.0,OPbO = 0.0,OTbO = 0.0,OThO = 0.0,TbO = 0.0,ThO = 0.0;
                int     Ticket = 0,ChB = 0,ChS = 0,IndEntry = 0;
                double  Pb = 0.0,Ph = 0.0,PaC = 0.0,PbPips = 0.0,PbTarget = 0.0,DrawDownPC = 0.0,BEb = 0.0,BEh = 0.0,BEa = 0.0;
                bool    BuyMe = 0.0,SellMe = 0.0,Success = 0.0,SetPOSL = 0.0;
                ...
                {              BEb=ND(BEb/LbT,Digits);
                                ...
                                {              FileHandle=FileOpen(FileName,FILE_BIN|FILE_WRITE);
                                                ...
                                                {              FileWriteInteger(FileHandle,TbF);
                                                                FileClose(FileHandle);
                                                                TradesOpen=true;
                                                                if(Debug)Print(FileName+" File Written: "+IntegerToString(TbF));
                                                }
                                }
                }
                else if(TradesOpen)
                ...
                {              if(CpT>0)
                                ...
                                {              SendMail("Blessing EA","Blessing has shut down on "+Symbol()+" "+sTF+
                                                ...
                                                Error=GetLastError();
                                                if(Error>0)Print("Error sending Email: "+IntegerToString(Error)+" "+ErrorDescription(Error));
                                                else EmailCount=4;
                                }
                }
                string Tab = "";
                ...
                //+----------------------------------------------------------------+
                double cci_01 = 0.0, cci_02 = 0.0, cci_03 = 0.0, cci_04 = 0.0, cci_11 = 0.0, cci_12 = 0.0, cci_13 = 0.0, cci_14 = 0.0;
                if(CCIEntry>0)
                ...
                {              if(displayOverlay)
                                ...
                                {              for(y=0;y<=175;y++)dSpace=dSpace+" ";
                                                ...
                                                {              if(!OrderSelect(y,SELECT_BY_POS,MODE_TRADES))continue;
                                                                ...
                                                                dMess=(dMess+"\n"+dSpace+" "+OrderTicket()+"  "+DTS(OrderMagicNumber(),0)+"   "+IntegerToString(OrderType()));
                                                ...
                                                {              dLabels=true;
                                                                ...
                                                                ObjSetTxt("B3VLotP",DTS(MarketInfo(Symbol(),MODE_LOTSTEP),IntegerToString(LotDecimal)));
...
{              if(FirstRun)return(-1);
                ...
                {              i+=1;
                                ...
                                Ticket=OrderSend(OSymbol,OCmd,OLot,OrderPrice,OSlip,0,0,"",OMagic,0,OColor);
                                if(Ticket<0)
                                {              Error=GetLastError();
...
{              double Equity=Balance+DrawDown;
                ...
                {              while(TimeNow>=NextStats)NextStats+=StatsPeriod;
                                ...
                                {              StatHandle=FileOpen(StatFile,FILE_WRITE|FILE_CSV,',');
                                                ...
                                                FileWrite(StatHandle,"Date","Time","Balance","Equity Low","Equity High","");
                                }
                                else
...
{              if(displayOverlay&&((Testing&&Visual)||!Testing))
                {              int dDigits;
                                ...
                                CreateLabel("B3LComm","Trade Comment: "+"",8-displayFontSize,5,5,1.8,displayColorFGnd,"Tahoma");
                                if(displayLogo)
