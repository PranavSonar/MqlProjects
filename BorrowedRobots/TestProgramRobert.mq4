...
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright "Robert Costache"
#property link      ""
...
void TestCloseProfit()
{
   ...
   {
      Ticket = OrderSelect(i, SELECT_BY_POS); //The function selects an order for further processing.
      if (OrderSymbol() == Symbol()) // Returns symbol name of the currently selected order.
      { // Returns the name of a symbol of the current chart.
...
void CloseAllOrders()
{
   ...
   for(int i=total-1;i>=0;i--)
   {
      ...
      { // Returns the name of a symbol of the current chart.
         if (OrderType() == OP_BUY)
            Ticket = OrderClose(OrderTicket(),OrderLots(),Bid,3,Yellow);
         else
            Ticket = OrderClose(OrderTicket(),OrderLots(),Ask,3,Yellow);
      }
   }
}
...
void CurentValue()
{
   ...
   for(int i=total-1;i>=0;i--)
   {
      ...
      if (OrderSymbol() == Symbol()) // Returns symbol name of the currently selected order.
      { // Returns the name of a symbol of the current chart.
         CV = CV + OrderProfit(); //Returns profit of the currently selected order.
      }
   }
   ...
   ObjectCreate("CV",OBJ_LABEL,0,0,0,0,0); // The function creates an object with the specified name, type, and the initial coordinates in the specified chart subwindow of the specified chart.
   ObjectSet("CV",OBJPROP_CORNER,1);
   ObjectSet("CV",OBJPROP_XDISTANCE,20); //Changes the value of the specified object property.
   ObjectSet("CV",OBJPROP_YDISTANCE,20);
   ObjectSetText("CV","CurrentValue = "+DoubleToString(CV,2),14,"Tahoma",text_color); //The function changes the object description.
}
